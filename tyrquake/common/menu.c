...
M_AdjustSliders(int dir)
{
	...
	break;
    case M_OPTIONS_CURSOR_JOYSTICKSPEED:
	joy_sensitivity.value += dir * 0.25f;
	joy_sensitivity.value = qclamp(joy_sensitivity.value, 0.25f, 3.0f);
	Cvar_SetValue("joy_sensitivity", joy_sensitivity.value);
	break;
...
M_Options_Draw(void)
{
    ...
    slider = (joy_sensitivity.value - 0.25f) / (3.0f - 0.25f);
    M_Print(16, height += 8, "        Joystick Speed");
    M_DrawSlider(220, height, slider);
...
typedef enum {
    ...
    M_KEYS_CURSOR_QUICKSAVE,
    M_KEYS_CURSOR_QUICKLOAD,
#ifdef TYRQUAKE_DEBUG_BUILD
    M_KEYS_CURSOR_NOCLIP,
    M_KEYS_CURSOR_GOD,
#endif
    M_KEYS_CURSOR_LINES,
} m_keys_cursor_t;
...
// Not static: our PS3 input driver (in_ps3.c) needs to check this, to
// avoid sending its own supplementary "back"/"confirm" key on top of
// whatever key is actively being captured for a rebind (see IN_Commands
// there for why).
int m_keys_bind_grab;
