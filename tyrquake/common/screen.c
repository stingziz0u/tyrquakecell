...
// draw a fresh screen
...
#ifdef TYRQUAKE_DEBUG_BUILD
    PS3_Log("SCR_ModalMessage: entering wait loop, key_lastpress=%d ('%c') key_count=%d before reset",
	    key_lastpress, (key_lastpress >= 32 && key_lastpress < 127) ? key_lastpress : '?', key_count);
#endif
	...
	iterations++;
#ifdef TYRQUAKE_DEBUG_BUILD
	if (iterations <= 10 || iterations % 500 == 0)
	    PS3_Log("SCR_ModalMessage: iteration %d, key_lastpress=%d ('%c')",
		    iterations, key_lastpress,
		    (key_lastpress >= 32 && key_lastpress < 127) ? key_lastpress : '?');
#endif
    } while (key_lastpress != 'y' && key_lastpress != 'n'
	     && key_lastpress != K_ESCAPE);

#ifdef TYRQUAKE_DEBUG_BUILD
    PS3_Log("SCR_ModalMessage: exited after %d iterations, key_lastpress=%d ('%c'), returning %d",
	    iterations, key_lastpress,
	    (key_lastpress >= 32 && key_lastpress < 127) ? key_lastpress : '?',
	    key_lastpress == 'y');
#endif
