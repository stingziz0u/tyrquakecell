...
BGM_PlayCDTrack(byte track, qboolean looping)
{
    ...
    PS3_Log("BGM_PlayCDTrack: track=%d looping=%d extmusic=%.0f disabled=%d",
	    (int)track, looping, bgm_extmusic.value, bgm_extmusic_disabled);

    if (bgm_extmusic.value && !bgm_extmusic_disabled) {
        int best_priority = INT_MAX;
        struct stream_params *bgm_type = NULL;
        for (int i = 0; i < ARRAY_SIZE(bgm_stream_types); i++) {
            const char *path = va("music/track%02d%s", (int)track, bgm_stream_types[i].extension);
            int priority = COM_FilePriority(path);
            PS3_Log("BGM_PlayCDTrack: checking '%s' -> priority=%d", path, priority);
        ...
        if (bgm_type) {
            ...
            PS3_Log("BGM_PlayCDTrack: calling BGM_Play('%s')", trackname);
            qboolean success = BGM_Play(trackname, looping);
            PS3_Log("BGM_PlayCDTrack: BGM_Play returned %d", success);
            if (success)
                return;
        } else {
            PS3_Log("BGM_PlayCDTrack: no matching file found for track %d in any supported format", (int)track);
        }
    }

    /* Otherwise, try CD Audio */
    PS3_Log("BGM_PlayCDTrack: falling back to CDAudio_Play (expected to no-op on this platform)");
