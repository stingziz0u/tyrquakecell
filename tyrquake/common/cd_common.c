...
CDAudio_SetVolume_f(struct cvar_s *var)
{
	...
	// Only attempt to sync back to real CD hardware volume when the
	// CD Audio subsystem is actually active. On platforms where it
	// never initializes (like this one -- no CD-ROM drive), enabled
	// stays false forever, CDDrv_SetVolume() always fails, and
	// cdvolume (which starts at 0.0 and is only ever updated on a
	// successful CDDrv_SetVolume call) would otherwise stay stuck at
	// 0 permanently -- snapping the "CD Music Volume" slider back to
	// minimum every single time, regardless of what value the player
	// actually sets it to via the OGG-music-backed bgmvolume slider.
	if (enabled) {
