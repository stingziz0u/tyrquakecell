...
// update auto-repeat status
	...
	key_repeats[key]++;
	if (key == K_ENTER)
	    PS3_Log("Key_Event: K_ENTER down, key_repeats=%d, key_dest=%d",
		    key_repeats[key], (int)key_dest);
	if (key != K_BACKSPACE
	    && key != K_PAUSE
	    && key != K_PGUP && key != K_PGDN && key_repeats[key] > 1) {
	    if (key == K_ENTER)
		PS3_Log("Key_Event: K_ENTER FILTERED as autorepeat (repeats=%d > 1)", key_repeats[key]);
