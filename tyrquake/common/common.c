...
//
	...
	if (search->pack) {
	    ...
	    //
	    // NOTE (PS3 port): same anti-piracy restriction as
	    // COM_FilePriority above (see comment there for why we
	    // deliberately don't enforce it here) -- this second copy is
	    // the one actually opening the file, so BGM_Play would find a
	    // music file exists (via COM_FilePriority) but still fail to
	    // open it here without this same fix.
...
COM_FilePriority(const char *filename)
{
	...
	if (search->pack) {
	    ...
	    // check a file in the directory tree
	    //
	    // NOTE (PS3 port): upstream skips this whole lookup for any
	    // path with a subdirectory ("/" in the name) on shareware
	    // installs -- a 1996-era anti-piracy measure meant to stop
	    // shareware players from sneaking in registered-game content
	    // via loose files. It has the side effect of blocking loose
	    // *music* files too (music/trackNN.ogg), which have nothing
	    // to do with that concern -- they're a separate, legitimately
	    // owned add-on, not registered game content. Deliberately not
	    // enforcing that restriction here.
	    qsnprintf(path, sizeof(path), "%s/%s", search->filename, filename);
...
// Diagnostic only -- dumps the full com_searchpaths chain (which
// directories/paks are active and in what priority order) to PS3_Log.
// com_searchpaths is static to this file, so this has to live here too
// rather than in sys_ps3.c, which is where the log line calling it
// actually sits (see Host_Init in NQ/host.c).
void
PS3_DumpSearchPaths(void)
{
    extern void PS3_Log(const char *fmt, ...);
    int i = 0;
    for (searchpath_t *s = com_searchpaths; s; s = s->next, i++) {
	if (s->pack)
	    PS3_Log("  searchpath[%d]: PAK %s (%d files)", i, s->pack->filename, s->pack->numfiles);
	else
	    PS3_Log("  searchpath[%d]: DIR %s", i, s->filename);
    }
    PS3_Log("PS3_DumpSearchPaths: %d entries total", i);
}
