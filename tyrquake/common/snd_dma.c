...
S_AllocBuffers()
{
    ...
    /* S_InitRamp() dereferences shm->speed, but shm is only valid if
     * SNDDMA_Init() (via S_Startup) actually succeeded. On platforms
     * where sound isn't wired up yet (like this PS3 port right now),
     * sound_started is correctly 0 here, but nothing was previously
     * guarding this call -- this is a latent bug in the stock code,
     * not something introduced by the PS3 port. */
    if (sound_started)
	S_InitRamp();
...
S_Init(void)
{
    ...
    S_Startup();
    PS3_Log("S_Init: S_Startup OK");
    SND_InitScaletable();
    PS3_Log("S_Init: SND_InitScaletable OK");
    S_AllocBuffers();
    PS3_Log("S_Init: S_AllocBuffers OK");
    ...
    S_InitAmbients();
    PS3_Log("S_Init: S_InitAmbients OK");

    S_StopAllSounds(true);
    PS3_Log("S_Init: S_StopAllSounds OK");

    S_Music_Init();
    PS3_Log("S_Init: S_Music_Init OK");
}
