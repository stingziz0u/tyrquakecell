...
Cvars_Init()
{
    ...
    PS3_Log("Cvars_Init: Sys_RegisterVariables OK");
    CDAudio_RegisterVariables();
    PS3_Log("Cvars_Init: CDAudio_RegisterVariables OK");
    COM_RegisterVariables();
    PS3_Log("Cvars_Init: COM_RegisterVariables OK");
    Con_RegisterVariables();
    PS3_Log("Cvars_Init: Con_RegisterVariables OK");
    Key_RegisterVariables();
    PS3_Log("Cvars_Init: Key_RegisterVariables OK");
    PR_RegisterVariables();
    PS3_Log("Cvars_Init: PR_RegisterVariables OK");
    NET_RegisterVariables();
    PS3_Log("Cvars_Init: NET_RegisterVariables OK");

    /* Note cls.state not set at this stage, so can't check for ca_dedicated */
    if (!COM_CheckParm("-dedicated")) {
        R_RegisterVariables();
        PS3_Log("Cvars_Init: R_RegisterVariables OK");
        Draw_RegisterVariables();
        PS3_Log("Cvars_Init: Draw_RegisterVariables OK");
        IN_RegisterVariables();
        PS3_Log("Cvars_Init: IN_RegisterVariables OK");
        SCR_RegisterVariables();
        PS3_Log("Cvars_Init: SCR_RegisterVariables OK");
        S_RegisterVariables();
        PS3_Log("Cvars_Init: S_RegisterVariables OK");
        VID_RegisterVariables();
        PS3_Log("Cvars_Init: VID_RegisterVariables OK");
        VID_Mode_RegisterVariables();
        PS3_Log("Cvars_Init: VID_Mode_RegisterVariables OK");
        Chase_RegisterVariables();
        PS3_Log("Cvars_Init: Chase_RegisterVariables OK");
        CL_RegisterVariables();
        PS3_Log("Cvars_Init: CL_RegisterVariables OK");
        V_RegisterVariables();
        PS3_Log("Cvars_Init: V_RegisterVariables OK");
    }

    SV_RegisterVariables();
    PS3_Log("Cvars_Init: SV_RegisterVariables OK");
}
...
Host_NewGame()
{
    extern void PS3_Log(const char *fmt, ...);
    Alpha_Init();
    PS3_Log("Host_NewGame: Alpha_Init OK");
    Draw_Init();
    PS3_Log("Host_NewGame: Draw_Init OK");
    SCR_Init();
    PS3_Log("Host_NewGame: SCR_Init OK");
    R_Init();
    PS3_Log("Host_NewGame: R_Init OK");
    Sbar_Init();
    PS3_Log("Host_NewGame: Sbar_Init OK");
}
...
Host_Init(quakeparms_t *parms, basedir_fn *basedir_fns)
{
    ...
    PS3_Log("Host_Init: Memory_Init OK");
    Cvars_Init();
    PS3_Log("Host_Init: Cvars_Init OK");
    Commands_Init();
    PS3_Log("Host_Init: Commands_Init OK");
    Cbuf_Init();
    PS3_Log("Host_Init: Cbuf_Init OK");

    V_Init();
    PS3_Log("Host_Init: V_Init OK");
    SV_InitLocal();
    PS3_Log("Host_Init: SV_InitLocal OK");
    Key_Init();
    PS3_Log("Host_Init: Key_Init OK");
    Con_Init();
    PS3_Log("Host_Init: Con_Init OK");
    M_Init();
    PS3_Log("Host_Init: M_Init OK");
    PR_Init();
    PS3_Log("Host_Init: PR_Init OK");
    Mod_Init(R_AliasModelLoader(), R_BrushModelLoader());
    PS3_Log("Host_Init: Mod_Init OK");
    NET_Init();
    PS3_Log("Host_Init: NET_Init OK");
    SV_Init();
    PS3_Log("Host_Init: SV_Init OK");

    Con_Printf("Exe: %s\n", Build_DateString());
    Con_Printf("%4.1f megabyte heap\n", parms->memsize / (1024 * 1024.0));
    PS3_Log("Host_Init: Con_Printf banner OK");

    R_InitTextures(); // needed even for dedicated servers (TODO: why? shouldn't be...)
    PS3_Log("Host_Init: R_InitTextures OK");

next_basedir:
    COM_InitFileSystem();
    PS3_Log("Host_Init: COM_InitFileSystem OK");
    COM_InitGameDirectoryFromCommandLine();
    PS3_Log("Host_Init: COM_InitGameDirectoryFromCommandLine OK");
    extern void PS3_DumpSearchPaths(void);
    PS3_DumpSearchPaths();
    ...
    if (cls.state != ca_dedicated) {
        ...
        PS3_Log("Host_Init: Host_LoadPalettes OK");

        VID_Init(host_basepal);
        PS3_Log("Host_Init: VID_Init OK");
        Host_NewGame();
        PS3_Log("Host_Init: Host_NewGame OK");

	S_Init();
	PS3_Log("Host_Init: S_Init OK");
	CDAudio_Init();
	PS3_Log("Host_Init: CDAudio_Init OK");
	CL_Init();
	PS3_Log("Host_Init: CL_Init OK");
	IN_Init();
	PS3_Log("Host_Init: IN_Init OK");
    }
    Mod_InitAliasCache();
    PS3_Log("Host_Init: Mod_InitAliasCache OK");

    Hunk_AllocName(0, "--HOST--");
    host_hunklevel = Hunk_LowMark();
    PS3_Log("Host_Init: Hunk_AllocName OK");
    ...
    if (!setjmp(host_abort)) {
	PS3_Log("Host_Init: about to exec quake.rc");
	Cbuf_InsertText("exec quake.rc\n");
	Cbuf_Execute();
	PS3_Log("Host_Init: exec quake.rc done");
    }
}
